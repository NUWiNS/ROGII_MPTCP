...
#define MAX_CAMERA_ID CAMERA_2 /*CAMERA_3*///ASUS BSP Ryan modify since yoda only have 3 camera
...
*/
	...
	if((camera_id == 0 || camera_id == 2) && !g_otp_data_override_done[camera_id])
	{
		...
		if(g_eeprom_info[0].mapdata == NULL || g_eeprom_info[0].num_data <= 0 )
		{
			...
			pr_err("g_eeprom_info[0].mapdata[6437] = 0x%02X \n", g_eeprom_info[0].mapdata[6437]);
		}else{
...
#define OV24B1Q_BIN_SIZE 0x1150
...
static void dual_cali_dump_create(void)
{
	if(g_eeprom_info[CAMERA_0].mapdata && g_eeprom_info[CAMERA_0].num_data > DUAL_CALI_END+1)
	{
		...
		proc_create_data(PROC_ARCSOFT_CALI, 0444, NULL, &dual_cali_fops, &g_eeprom_info[CAMERA_0]);
	}
	else
...
static int cover_dit_cal_data(struct cam_eeprom_ctrl_t *e_ctrl,uint32_t camera_id)
{
	...
	if(num>0)
	{
		pr_info("[DIT_EEPROM]  read %d bytes from %s camera_id = %d",num,load_file_name,camera_id);
	...
	{
		pr_info("[DIT_EEPROM] read vendor/factory file fail,  read %d bytes from golden file %s camera_id = %d",num,dit_eeprom_golden_file_name[camera_id],camera_id);
	}else{
		pr_info("[DIT_EEPROM] read %s failed, rc %d",dit_eeprom_file_name[camera_id],num);
	}
	if(camera_id == CAMERA_0 /*|| camera_id == CAMERA_1*/)//ASUS BSP Ryan remove cam 1
	{
...
//ASUS_BSP Ryan ++ combine dut_front.bin & eeprom data to dit_rear_eeprom
	else if (camera_id == CAMERA_1){
		if(num<=0)
		{/*If there is no dit bin ,set user space eeprom data to 0*/
			pr_err("ryan eeprom num=%d",num);
			memset(e_ctrl->cal_data.mapdata,0, e_ctrl->cal_data.num_data);
		}
		/*dut_front.bin pccupy 4432 byte(OV24B1Q_BIN_SIZE) copy real eeprom map data from 0x1150
		 only copy OV24B1Q_EEPROM_MAP_SIZE byte to dit_rear_eeprom for qcom use */
		memcpy(e_ctrl->cal_data.mapdata + OV24B1Q_BIN_SIZE, g_eeprom_info[camera_id].mapdata + OV24B1Q_EEPROM_MAP_START, OV24B1Q_EEPROM_MAP_SIZE);	
	}
//ASUS_BSP Ryan --combine dut_front.bin & eeprom data to dit_rear_eeprom
...
{
	if(camera_id == CAMERA_0 || camera_id == CAMERA_1 /*ASUS BSP ryan ++ FOR Cam 1 eeprom*/)
	{
...
void eeprom_dump_create(struct cam_eeprom_ctrl_t * e_ctrl)
{
	...
	{
		pr_err("ryan eeprom_dump_create free camera_id=%d",camera_id);
	...
	if(g_eeprom_ctrl[camera_id] == NULL)//first time
	{
		...
		otp_name = g_otp_proc_file_name[camera_id];//ASUS BSP ryan ++ FOR Cam 1 eeprom
		/*double check cameraid and module id for create otp*/
		if((camera_id == CAMERA_0 && (g_eeprom_info[camera_id].mapdata[8] == 0x66 || g_eeprom_info[camera_id].mapdata[8] == 0x69))|| 
			(camera_id == CAMERA_1 && g_eeprom_info[camera_id].mapdata[8] == 0x67))//Asus BSP Ryan++ check module id :0x66 & 0x69  for cam0 /  0x67 for cam1
		{
			create_proc_file(file_name,&eeprom_proc_fops,&g_eeprom_info[camera_id].id);
			create_proc_file(otp_name,&eeprom_to_otp_fops,&g_eeprom_info[camera_id].id); // eeprom to otp
...
void asus_cam_sensor_init(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if(!g_sensor_init_state[s_ctrl->id])
	{
		...
		if(s_ctrl->id == CAMERA_2 /*ryan remove || s_ctrl->id == CAMERA_3*/)
		{
