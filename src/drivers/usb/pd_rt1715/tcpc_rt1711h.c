...
printk("%s: Set rt1711 pinctrl state\n", __func__);
	...
	if (g_ASUS_hwID == ZS660KL_EVB && g_ASUS_prjID == ZS660KL_PRJ_L)
		rt1711_pinctrl_state_active = pinctrl_lookup_state(rt1711_pinctrl, "rt1715_int_active_EVB3");
	else
...
static int rt_parse_dt(struct rt1711_chip *chip, struct device *dev)
{
	...
	pr_info("[Bottom_PD] %s g_ASUS_hwID is %d, g_ASUS_prjID is %d\n", __func__, g_ASUS_hwID, g_ASUS_prjID);
