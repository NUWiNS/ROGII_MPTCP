...
#define DRIVER_VERSION "1.3.1b06"
...
static struct ghci_urb *ghci_alloc_urb(gfp_t mem_flags)
{
	...
	memset(hcurb, 0x00, sizeof(*hcurb));
...
static struct ghci_urb *ghci_create_ext_urb(
	struct ghci_hcd *ghci, gfp_t mem_flags, int ext_flags)
{
	struct ghci_urb *hcurb;
	struct ghci_urb_header *hdr;
	...
	case USB_REQ_SET_ADDRESS:
		...
		if (retval == 0) {
			dev_dbg(hcd->self.controller,
				...
				dev->devnum, port_index + 1, hcd_to_pdev(hcd)->id);
		} else {
			dev_err(hcd->self.controller,
				"SET_ADDRESS %u failed, port#%d hcd#%d\n",
				le16_to_cpu(setup_packet->wValue),
				port_index + 1, hcd_to_pdev(hcd)->id);
		}
		break;
 ...
 * Caller must have a private hcd lock.
 */
static void ghci_complete_ext(struct ghci_hcd *ghci,
	struct ghci_urb *hcurb)
{
	struct ghci_device *dev = hcurb->dev;

	if (dev->port == NULL) {
		dev_info(ghci_to_hcd(ghci)->self.controller,
			"the device is already removed on"
			" completion of Extended Function (0x%04x)\n",
			ghci_urb_function(hcurb->header.type));
		return;
	}

	switch (ghci_urb_function(hcurb->header.type)) {
	case (GHCI_URB_EXT | GHCI_EXT_PORT_RESET):
		/* USB Port Reset is completed */
		if (hcurb->header.status) {
			/* ignore the server error */
			dev_warn(ghci_to_hcd(ghci)->self.controller,
				"Port Reset gets error (resut 0x%04x)\n",
				hcurb->header.status);
		}
		if (!(dev->port->status & USB_PORT_STAT_RESET)) {
			dev_warn(ghci_to_hcd(ghci)->self.controller,
				"Do not reset address on the completion of Port Reset(status 0x%04x)",
				dev->port->status);
		}
		else {
			/* Change port status/ Reset address to clear port */
			dev->port->status &= ~USB_PORT_STAT_RESET;
			ghci_reset_address(dev);
			dev->port->status |= USB_PORT_STAT_ENABLE;

			dev_info(ghci_to_hcd(ghci)->self.controller,
				"complete Extended Function (0x%04x)\n",
				ghci_urb_function(hcurb->header.type));
		}
		break;
	...
	int status)
{
	...
	/* Extended Function uses GHCI URB only *
	 * and it does not have any urbs        */
	if (!urb) {
		ghci_complete_ext(ghci, hcurb);
	}
	ghci_free_urb(hcurb);
...
static int ghci_dev_readable(struct ghci_port *port)
{
	...
	if (dev == NULL) {
		retval = -ESHUTDOWN;
		dev_err(ghci_to_hcd(ghci)->self.controller,
...
#ifdef DEBUG
			printk(KERN_DEBUG "%s EFAULT dst %p src %p len %zd/%zu\n",
	...
	loff_t *pos)
{
	...
	retval = wait_event_interruptible(
		ghci_drv.read_wait, ((readable = ghci_dev_readable(port)) != 0));
	if (retval < 0 || readable < 0) {
		if (retval >= 0) {
			retval = readable;
		}
		return retval;
	...
	loff_t *pos)
{
	...
	if (dev == NULL) {
		retval = -ESHUTDOWN;
		dev_err(hcd->self.controller,
			"%s: The device may be already removed\n",
			__func__);
		goto done;
	}
...
static int ghci_connect_device(struct ghci_port *port, struct ghci_device *dev)
{
	...
	port->ghci_dev = dev;
	/* set ignore flag not to do port reset on connecting device */
	port->flags |= GHCI_FLAG_RESET_IGNORE;
	dev->port = port;
	...
	dev_info(hcd->self.controller,
		...
		port->port_index + 1, hcd_to_pdev(hcd)->id);
 ...
 * Caller must have a private hcd lock.
 */
static struct ghci_device *ghci_disconnect_port(struct ghci_port *port)
{
	struct ghci_hcd *ghci = port->ghci;
	struct usb_hcd *hcd = ghci_to_hcd(ghci);
	struct ghci_device *dev;
 ...
 * Caller must have a private hcd lock.
 */
static int ghci_disconnect_device(struct usb_hcd *hcd,
	struct ghci_hcd *ghci, struct ghci_port *port)
{
	struct ghci_device *dev;
	int retval = 0;

	dev = ghci_disconnect_port(port);
	if (!dev) {
		retval = -ENODEV;
		dev_err(hcd->self.controller,
...
static int ghci_disconnect_all_devices(void) {
	struct usb_hcd *hcd;
	struct ghci_platform_hcd *priv;
	struct ghci_hcd *ghci;
	struct ghci_port *port;
	...
	for (i = 0; i < ARRAY_SIZE(ghci_drv.pdev); i++) {
		...
		for (j = 0; j < priv->nr_ports; j++) {
			...
			dev_info(hcd->self.controller,
				"disconncet device (port#%d) to suspend\n", j + 1);
			retval = ghci_disconnect_device(hcd, ghci, port);
			if (retval < 0) {
				if (retval != -ENODEV) {
					break;
				}
				/* ENODEV is not error here */
				retval = 0;
			}
		}
		spin_unlock_irqrestore(&ghci->lock, flags);
...
static int ghci_dev_open(struct inode *inode, struct file *file)
{
	...
	if (ghci_drv.power & GHCI_POWER_INACTIVE) {
		dev_info(ghci_drv.ctl_device,
			"power is not active 0x%04x, do not open device\n",
			ghci_drv.power);
		return -EBUSY;
	}
	...
	if (retval < 0) {
		dev_err(hcd->self.controller,
			...
			port->port_index + 1, hcd_to_pdev(hcd)->id, retval);
		goto done;
	}
...
static int ghci_urb_enqueue_ext(struct ghci_hcd *ghci,
	struct ghci_device *dev, int ext_flags)
{
	struct ghci_urb *hcurb;
	unsigned long flags;

	dev_info(ghci_to_hcd(ghci)->self.controller,
		"enqueue Extended Function (0%04x)\n", ext_flags);

	hcurb = ghci_create_ext_urb(ghci, GFP_KERNEL, ext_flags);
	if (!hcurb) {
		dev_err(ghci_to_hcd(ghci)->self.controller,
...
#endif
{
	...
	if (retval < 0) {
		dev_err(hcd->self.controller,
			"check unlink failure status %d, err %d\n",
			status, retval);
		spin_unlock_irqrestore(&ghci->lock, flags);
		return retval;
	}
	...
	u16 wLength)
{
	...
	case GetPortStatus:
		...
		if ((ghci->port[i].status & USB_PORT_STAT_RESET) &&
			((ghci->port[i].flags & GHCI_FLAG_RESET_IGNORE) ||
			(ghci->port[i].ghci_dev == NULL))) {
			/* Emulate the status handling for port reset *
			 * if the port reset request is ignored       */
			dev_info(hcd->self.controller,
				"emulate completion of USB Port Reset (port %d)\n", i + 1);
			...
			/* clear ignore flag to create reset requests after enumeration */
			ghci->port[i].flags &= ~GHCI_FLAG_RESET_IGNORE;
		}
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_LINK_STATE:
			...
			/*
			 * This Virtual Host Controller cannot handle Link State.
			 * So there is nothing to do.
			 */
			dev_info(hcd->self.controller,
				"USB_PORT_FEAT_LINK_STATE: port %d, value 0x%x\n",
				i + 1, wIndex & 0xff00);
			break;
		...
		case USB_PORT_FEAT_RESET:
			if ((ghci->port[i].flags & GHCI_FLAG_RESET_IGNORE) ||
				(ghci->port[i].ghci_dev == NULL)) {
				dev_info(hcd->self.controller,
					"emulate SetPortFeature USB Port Reset (port %d)\n", i + 1);
			}
			else {
				/* Send USB Port Reset Request */
				retval = ghci_urb_enqueue_ext(ghci, ghci->port[i].ghci_dev,
					GHCI_URB_EXT | GHCI_EXT_PORT_RESET);
				if (retval < 0) {
					dev_err(hcd->self.controller,
					"cannot enqueue USB Port Reset (retval %d)\n", retval);
					goto error;
				}
			}
			/* FALLS THROUGH */
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 48)
static dma_addr_t ghci_sec_event_ring_dma_add(struct usb_hcd *hcd,
	unsigned intr_num)
#else
static phys_addr_t ghci_sec_event_ring_phys_add(struct usb_hcd *hcd,
	unsigned intr_num, dma_addr_t *dma)
#endif
{
	dev_info(hcd->self.controller, "%s return 0\n", __func__);

	return 0;
}

#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 48)
static dma_addr_t ghci_get_xfer_ring_dma_addr(struct usb_hcd *hcd,
	struct usb_device *udev, struct usb_host_endpoint *ep)
#else
static phys_addr_t ghci_get_xfer_ring_phys_addr(struct usb_hcd *hcd,
	struct usb_device *udev, struct usb_host_endpoint *ep,
	dma_addr_t *dma)
#endif
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 48)
static dma_addr_t ghci_get_dcba_dma_addr(struct usb_hcd *hcd,
...
#endif
#endif /* CONFIG_SND_USB_AUDIO_QMI */
...
#ifdef CONFIG_SND_USB_AUDIO_QMI
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 48)
	.get_sec_event_ring_dma_addr = ghci_sec_event_ring_dma_add,
	.get_xfer_ring_dma_addr = ghci_get_xfer_ring_dma_addr,
	.get_dcba_dma_addr = ghci_get_dcba_dma_addr,
#else
	.get_sec_event_ring_phys_addr = ghci_sec_event_ring_phys_add,
	.get_xfer_ring_phys_addr = ghci_get_xfer_ring_phys_addr,
#endif
#endif /* CONFIG_SND_USB_AUDIO_QMI */
};
...
static int ghci_pm_prepare(struct device *dev)
{
	int retval;

	ghci_drv.power |= GHCI_POWER_SUSPENDING;

	dev_dbg(ghci_drv.ctl_device,
		"prepare to suspend (ghci power state 0x%04x)\n",
		ghci_drv.power);
	retval = ghci_disconnect_all_devices();

	return retval;
}

static void ghci_pm_complete(struct device *dev)
{
	ghci_drv.power &= ~GHCI_POWER_SUSPENDING;

	dev_dbg(ghci_drv.ctl_device,
		"resume completed (ghci power state 0x%04x)\n",
		ghci_drv.power);

	return;
}

static int ghci_pm_suspend(struct device *dev)
{
	ghci_drv.power |= GHCI_POWER_SUSPENDED;

	dev_dbg(ghci_drv.ctl_device,
		"suspended (ghci power state 0x%04x)\n",
		ghci_drv.power);

	return 0;
}

static int ghci_pm_resume(struct device *dev)
{
	ghci_drv.power &= ~GHCI_POWER_SUSPENDED;

	dev_dbg(ghci_drv.ctl_device,
		"resumed (ghci power state 0x%04x)\n",
		ghci_drv.power);

	return 0;
}

static struct dev_pm_ops ghci_dev_pm_ops = {
	.prepare = ghci_pm_prepare,
	.complete = ghci_pm_complete,
	SET_SYSTEM_SLEEP_PM_OPS(ghci_pm_suspend, ghci_pm_resume)
};
		...
		for (j = 0; j < priv->nr_ports; j++) {
			...
			if (!port->claimed && !port->ghci_dev) {
				...
				dev_dbg(hcd->self.controller,
					...
					port->port_index + 1, port->dev_index);
	...
	struct platform_device **new_pdev)
{
	...
	/* some platform set DMA mask forcibly */
	if (pdev->dev.dma_mask) {
		printk(KERN_INFO "gxhci: invalid DMA configuration detects (%p: 0x%llx)\n",
			pdev->dev.dma_mask, *pdev->dev.dma_mask);
		/* Overwrite the value and the pointer not to use dma allocation.     *
		 * Make sure the pointer of dma_mask is not dynamic allocated memory. */
		*pdev->dev.dma_mask = 0;
